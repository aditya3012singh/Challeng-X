...
export const BattleProblem = ({ problem }) => {
  ...
  if (!problem) {
    return (
      <div className="px-6 py-6 space-y-6 bg-[#080808] border-r border-[#1e1e1e] min-h-screen pt-12 shadow-md">
        <h2 className="text-[var(--color-primary)] font-black tracking-widest uppercase text-sm mb-8">CodeArena</h2>
  ...
  return (
    <div className="px-6 py-6 h-full overflow-y-auto bg-[#080808] border-r border-[#1e1e1e] min-h-screen pt-12 shadow-md pb-24">
      <h2 className="text-[var(--color-primary)] font-black tracking-widest uppercase text-sm mb-8">CodeArena</h2>

      <h1 className="text-3xl font-black mb-3 text-white tracking-tight">{problem.title}</h1>
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-[2px] p-6 my-6 border-l-4 border-l-[var(--color-primary)]">
        <h2 className="text-sm font-bold tracking-widest uppercase mb-3 text-slate-400">Description</h2>
      ...
      {problem.constraints && (
        <div className="bg-white/[0.02] border border-white/5 rounded-[2px] p-6 mb-6">
          <h2 className="text-sm font-bold tracking-widest uppercase mb-3 text-slate-400">Constraints</h2>
          <pre className="text-slate-300 text-sm font-mono whitespace-pre-line bg-[#050505] p-4 border border-white/5">
            {problem.constraints}
          </pre>
        </div>
      )}
      ...
      {problem.testcases && problem.testcases.filter(tc => tc.isSample || !tc.isHidden).length > 0 && (
        <div className="bg-white/[0.02] border border-white/5 rounded-[2px] p-6 mb-6">
          <h2 className="text-sm font-bold tracking-widest uppercase mb-4 text-slate-400">Sample Cases</h2>
          <div className="space-y-4">
            {problem.testcases
              ...
              .map((testcase, index) => (
                <div
                  key={index}
                  className="border border-white/5 bg-[#050505] p-4 rounded-[2px]"
                >
                  <h3 className="font-bold mb-3 text-slate-300 text-xs uppercase tracking-widest">
                    Example {index + 1}
                  </h3>
                  <div className="mb-3">
                    <strong className="text-[10px] uppercase tracking-widest text-slate-500 block mb-2">Input:</strong>
                    <pre className="bg-white/[0.02] border border-white/5 p-3 text-slate-300 font-mono text-sm rounded-[2px] overflow-x-auto">
                      {testcase.input}
                    </pre>
                  </div>
                  <div>
                    <strong className="text-[10px] uppercase tracking-widest text-slate-500 block mb-2">Expected Output:</strong>
