        ...
        <div className="premium-card p-12 lg:p-16 relative overflow-hidden" style={{ borderRadius: "2px" }}>
          <div className="flex flex-col md:flex-row items-center justify-between mb-16 gap-8">
            <div>
              ...
              <h3 className="text-4xl font-black text-[var(--color-text-main)] uppercase tracking-tighter">
                {currentBattle.team1?.name} <span className="text-slate-600 px-4 font-mono">VS</span> {currentBattle.team2?.name}
              </h3>
            </div>
            <div className="text-center md:text-right">
              <div className="text-[9px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-2">Synchronization Key</div>
              <div className="font-mono text-xl font-black text-[var(--color-text-main)] tracking-widest mb-4 bg-white/[0.02] px-6 py-2 border border-white/5 inline-block">{currentBattle.battleCode}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-20">
            <div className="p-10 border-l-4 border-[var(--color-primary)] bg-white/[0.01] shadow-2xl relative group">
              <div className="absolute top-4 right-6 text-[9px] font-bold text-slate-600 uppercase tracking-widest group-hover:text-[var(--color-text-main)] transition-colors">Cluster One</div>
              <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-[0.3em] mb-4">{currentBattle.team1?.name}</p>
              <div className="flex items-baseline gap-4">
                <p className="text-7xl font-black text-[var(--color-text-main)] tabular-nums">{currentBattle.team1Wins || 0}</p>
            ...
            <div className="p-10 border-l-4 border-white/10 bg-white/[0.01] shadow-2xl relative group">
              <div className="absolute top-4 right-6 text-[9px] font-bold text-slate-600 uppercase tracking-widest group-hover:text-[var(--color-text-main)] transition-colors">Cluster Two</div>
              <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-[0.3em] mb-4">{currentBattle.team2?.name}</p>
              <div className="flex items-baseline gap-4">
                <p className="text-7xl font-black text-[var(--color-text-main)] tabular-nums">{currentBattle.team2Wins || 0}</p>
                <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Wins Recorded</p>
              </div>
            </div>
          </div>
          ...
          {/* Matches Grid */}
          <div className="mb-16">
            <h4 className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-[0.5em] mb-8 border-b border-white/5 pb-4">Logic Duels // 1V1</h4>
          ...
          <div className="pt-12 border-t border-white/[0.03]">
            ...
            {currentBattle.status === 'active' && (
              <button
                ...
                disabled={battleLoading}
                className="w-full py-6 border border-white/10 text-[var(--color-text-main)] font-bold uppercase tracking-widest text-xs hover:border-white hover:bg-white/5 transition-all"
                style={{ borderRadius: "2px" }}
              >
            ...
            {currentBattle.status === 'completed' && (
              <div className="p-10 bg-white/[0.01] border border-[var(--color-primary)]/20 text-center" style={{ borderRadius: "2px" }}>
                <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-[0.3em] mb-4">Official Result</p>
